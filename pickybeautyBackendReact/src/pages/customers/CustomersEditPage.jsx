...
function CustomersEditPage() {
  ...
  useEffect(() => {
    const fetchCustomer = async () => {
      try {
        const res = await fetch(`${backendUrl}/wp-json/kibsterlp-admin/v1/customers/${id}`, {
          headers: {
        ...
        if (!res.ok) {
          ...
          showErrorToast(err.message || "Failed to load customer");
          setLoading(false);
          return;
        }
        ...
        setFormData({
          ...
          email: cust.email || "",
          phone: cust.phone || "",
          order_complete_count: cust.order_complete_count ?? "",
          order_cancelled_count: cust.order_cancelled_count ?? "",
          order_complete_percentage: cust.order_complete_percentage ?? "",
        });
      } catch (err) {
        showErrorToast("Failed to fetch customer details.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        const err = await res.json();
        showErrorToast(err.message || "Failed to update customer");
        return;
      }
      ...
      showErrorToast("Something went wrong while updating the customer.");
    }
  };
